...
// Issue #162 keyboard acceptance: exercises the real Tab / Shift+Tab / Space
// keyboard contract of QualityDisclosure (never via .focus() as a Tab
// substitute), on top of the deferred-autofocus race fix. Complements —
// does not replace or modify — the existing Enter test above.
test.describe('quality disclosure real keyboard navigation (issue #162 acceptance)', () => {
  test('Tab moves focus onto the close button without the deferred autofocus reclaiming it', async ({ page, isMobile }) => {
    ...
    // Captured after opening (not before): opening a below-the-fold trigger
    // legitimately scrolls it into view first, same rationale as the
    // regression #156 test above.
  ...
  test('Shift+Tab moves focus backward without landing on <body> or the dialog container', async ({ page, isMobile }) => {
    ...
    // unrelated, distant element by design, not the trigger. That is a
    // pre-existing characteristic of the portal/non-modal architecture,
    // identical before and after this fix, and out of scope for the #162
    // autofocus-race contract. What IS in scope and asserted below: focus
    // must not land on <body>, and — the actual regression check — the
    // deferred autofocus must not have reclaimed it back onto the dialog
    // container itself; the disclosure itself must remain open (correct
    // non-modal behavior: tabbing out does not dismiss it).
    expect(await page.evaluate(() => document.activeElement?.tagName)).not.toBe('BODY');
