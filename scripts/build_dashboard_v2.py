...
def main() -> None:
    ...
    (DIST / "assets" / "styles").mkdir(parents=True, exist_ok=True)
    (DIST / "assets" / "components").mkdir(parents=True, exist_ok=True)
    (DIST / "data").mkdir(parents=True, exist_ok=True)
    for name in STYLE_FILES:
        shutil.copy2(SRC / "styles" / name, DIST / "assets" / "styles" / name)
    for name in SCRIPT_FILES:
        shutil.copy2(SRC / "components" / name, DIST / "assets" / "components" / name)
    shutil.copy2(SRC / "app.js", DIST / "assets" / "app.js")
    shutil.copy2(ROOT / "data" / "aggregates" / "dashboard_data.json", DIST / "data" / "dashboard_data.json")
    (DIST / "index.html").write_text(html, encoding="utf-8")
    (ROOT / "index.html").write_text(html, encoding="utf-8")
    print(f"built {ROOT / 'index.html'} ({len(html):,} chars)")
