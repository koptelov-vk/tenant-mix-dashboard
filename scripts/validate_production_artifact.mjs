...
const required = ['index.html', 'build-info.json', 'data/dashboard_data.json'];
...
for (const file of files) {
  if (forbiddenName.test(file)) fail(`forbidden legacy or transitional file found: ${file}`);
}
...
try {
  ...
  fail(`build-info.json is not valid JSON: ${error.message}`);
}
...
console.log(`Production artifact validated: ${files.length} files, one canonical index, build ${buildInfo.build}`);
