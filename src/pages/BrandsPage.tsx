import { Download, ExternalLink, FileSpreadsheet, Filter, RotateCcw, Search, X } from 'lucide-react';
...
export default function BrandsPage({ context, data }: { context: AnalysisContext; data: DashboardData }) {
  ...
  const options = useMemo(() => ({ malls: unique(context.filteredRows.map((row) => row.mall)), cities: unique(context.filteredRows.map((row) => cityByMall.get(row.mall) ?? row.city)), categories: unique(context.filteredRows.map((row) => row.category)), sources: unique(context.filteredRows.map((row) => row.sourceType || 'н/д')), qualities: unique(context.filteredRows.map((row) => row.sourceQuality || 'н/д')) }), [context.filteredRows, cityByMall]);
  const rows = useMemo(() => {
...
function unique(values: string[]) { return [...new Set(values)].sort((a, b) => a.localeCompare(b, 'ru', { numeric: true })); }
function MultiFilter({ label, options, value, onChange }: { label: string; options: string[]; value: string[]; onChange: (value: string[]) => void }) {
  const toggle = (option: string) => onChange(value.includes(option) ? value.filter((item) => item !== option) : [...value, option]);
  return <details className="registry-filter"><summary><Filter size={14} />{label}{value.length ? <b>{value.length}</b> : null}</summary><div><div className="registry-filter-tools"><button onClick={() => onChange(options)}>Выбрать все</button><button onClick={() => onChange([])}>Снять все</button></div>{options.map((option) => <label key={option}><input type="checkbox" checked={value.includes(option)} onChange={() => toggle(option)} /><span>{option}</span></label>)}</div></details>;
}
