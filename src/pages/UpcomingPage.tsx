...
const headers: Array<[SortKey, string]> = [['mall', 'Объект'], ['brand', 'Бренд'], ['category', 'Категория'], ['publication', 'Публикация'], ['planned', 'Плановая дата'], ['checked', 'Проверено'], ['status', 'Статус'], ['source', 'Источник']];
...
export default function UpcomingPage({ context, data }: { context: AnalysisContext; data: DashboardData }) {
  ...
  return <><div className="page-heading"><div><h1>Скоро открытие</h1><p>{rows.length} из {baseRows.length} подтверждённых анонсов в текущем срезе. Просроченная плановая дата не означает автоматическое открытие.</p></div></div>
    <Card className="upcoming-panel unified-table-panel"><div className="registry-toolbar upcoming-toolbar"><label className="search-field"><Search size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Поиск по объекту, бренду, категории или статусу" aria-label="Поиск в таблице скоро открытие" />{search ? <button onClick={() => setSearch('')} aria-label="Очистить поиск"><X size={16} /></button> : null}</label><span className="registry-result-count" role="status" aria-live="polite"><strong>{rows.length}</strong> анонсов</span></div>
