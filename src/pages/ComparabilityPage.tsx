...
export default function ComparabilityPage({ context, data }: { context: AnalysisContext; data: DashboardData }) {
  ...
  const rows = useMemo(() => {
    ...
    const peers = context.mallStats.filter((item) => item.mall.mall !== context.focusMall.mall);
  ...
  }, [context.mallStats, context.focusMall.mall, context.similarities, sort]);
  ...
  return <><div className="page-heading"><div><h1>Сопоставимость объектов</h1><p>Сравнение масштаба, состава брендов и структуры tenant mix текущего среза.</p></div><div className="segmented"><Button variant={mode === 'table' ? 'default' : 'outline'} onClick={() => setMode('table')}>Таблица</Button><Button variant={mode === 'cards' ? 'default' : 'outline'} onClick={() => setMode('cards')}>Карточки</Button></div></div>
    <Card className="comparison-table-panel">{mode === 'table' ? <div className="table-scroll"><table className="data-table comparison-table"><caption className="sr-only">Сравнение характеристик и состава брендов объектов</caption><thead><tr>{headers.map(([key, label, unit]) => <th key={key} className={numericKeys.has(key) ? 'numeric-column' : ''} aria-sort={sort?.key === key ? (sort.direction === 'asc' ? 'ascending' : 'descending') : 'none'}><button className="sort-header" onClick={() => toggleSort(key)}><span>{label}{unit ? <small>{unit}</small> : null}</span>{sort?.key === key ? (sort.direction === 'asc' ? <ArrowUp size={14} /> : <ArrowDown size={14} />) : <ArrowUpDown size={14} />}</button></th>)}</tr></thead><tbody>{rows.map((item) => { const sim = similarity.get(item.mall.mall); const isFocus = item.mall.mall === context.focusMall.mall; return <tr key={item.mall.mall} className={isFocus ? 'focus-row' : ''}><th><button className="table-link" onClick={() => setSelectedMall(item.mall.mall)}>{item.mall.mall}</button>{isFocus ? <small>Фокусный объект</small> : null}</th><td>{item.mall.city}</td><td><span className="class-label">{item.mall.mallClass}</span></td><td className="numeric-column">{item.mall.glaConfirmed ? formatArea(item.mall.gla) : 'н/д'}</td><td className="numeric-column">{formatArea(item.mall.gba)}</td><td className="numeric-column"><strong>{item.brandCount}</strong></td><td className="numeric-column">{item.density10kGla == null ? 'н/д' : formatNumber.format(item.density10kGla)}</td><td className="numeric-column">{sim ? formatPercent(sim.jaccard) : '—'}</td><td className="numeric-column">{sim?.common ?? '—'}</td></tr>; })}</tbody></table></div> : <div className="mall-card-grid">{rows.map((item) => { const sim = similarity.get(item.mall.mall); return <article key={item.mall.mall} className={item.mall.mall === context.focusMall.mall ? 'mall-card focus' : 'mall-card'}><button className="mall-card-title" onClick={() => setSelectedMall(item.mall.mall)}>{item.mall.mall}</button><span>{item.mall.city} · {item.mall.mallClass}</span><dl><div><dt>Бренды</dt><dd>{item.brandCount}</dd></div><div><dt>GLA</dt><dd>{item.mall.glaConfirmed ? formatArea(item.mall.gla) : 'н/д'}</dd></div><div><dt>Жаккар</dt><dd>{sim ? formatPercent(sim.jaccard) : '—'}</dd></div></dl></article>; })}</div>}</Card>
