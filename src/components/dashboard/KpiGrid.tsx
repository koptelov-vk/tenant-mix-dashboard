...
export function KpiGrid({ context }: { context: AnalysisContext }) {
  ...
  const methodTrigger = useRef<HTMLButtonElement>(null);
  const methodContent = useRef<HTMLElement>(null);
  const methodOverlay = useControlledOverlay({ open: showMethod, setOpen: setShowMethod, triggerRef: methodTrigger, contentRef: methodContent });
