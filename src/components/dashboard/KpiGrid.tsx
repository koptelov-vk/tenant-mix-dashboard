...
export function KpiGrid({ context }: { context: AnalysisContext }) {
  ...
  const items = [
    ...
    { label: 'Пересечение с группой', value: formatNumber.format(intersectionCount), note: `${formatPercent(focusCount ? intersectionCount / focusCount : 0)} брендов объекта`, formula: 'Бренды фокусного ТЦ, представленные хотя бы у одного выбранного конкурента.' },
