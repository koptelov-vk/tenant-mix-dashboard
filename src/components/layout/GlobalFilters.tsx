...
export function GlobalFilters({ data, context }: { data: DashboardData; context: AnalysisContext }) {
  ...
  return <section className="filter-shell" aria-label="Параметры анализа">
    <div className="filter-bar">
      ...
      <div className="filter-actions"><Button variant="outline" onClick={() => setMallSelector(true)}><Check size={17} />Объекты сравнения <b className="filter-count">{context.peerMalls.length}</b></Button><Button variant="outline" onClick={() => setAdvanced(!advanced)} aria-expanded={advanced} aria-label="Расширенные фильтры"><Filter size={17} />Ещё{active ? <b className="filter-count">{active}</b> : null}</Button></div>
    </div>
