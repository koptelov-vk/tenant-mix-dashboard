...
export function GlobalFilters({ data, context }: { data: DashboardData; context: AnalysisContext }) {
  ...
  const active = [state.category !== 'Все категории', state.cities.length > 0, state.sourceQualities.length > 0, state.glaMin != null, state.glaMax != null, state.gbaMin != null, state.gbaMax != null].filter(Boolean).length;
  ...
  return <section className="filter-shell" aria-label="Параметры анализа">
    <div className="filter-bar">
      ...
      <label><span>Группа сравнения</span><select value={state.peerGroup} onChange={(event) => applyPreset(event.target.value as 'same-class' | 'all')}><option value="same-class">Тот же класс</option><option value="all">Все объекты</option>{state.peerGroup === 'custom' ? <option value="custom">Ручной состав</option> : null}</select></label>
      ...
      <div className="filter-actions"><Button variant="outline" onClick={() => setMallSelector(true)}><Check size={17} />Объекты сравнения <b className="filter-count">{context.peerMalls.length}</b></Button><Button variant="outline" onClick={() => setAdvanced(!advanced)} aria-expanded={advanced}><Filter size={17} />Ещё{active ? <b className="filter-count">{active}</b> : null}</Button></div>
    </div>
    ...
    {mallSelector ? <MallSelector data={data} focusMall={context.focusMall.mall} selected={state.peerGroup === 'custom' ? state.selectedMalls : context.peerMalls.map((mall) => mall.mall)} onClose={() => setMallSelector(false)} onApply={(malls) => { state.setSelectedMalls(malls); setMallSelector(false); }} /> : null}
  </section>;
}
