...
export function GlobalFilters({ data, context }: { data: DashboardData; context: AnalysisContext }) {
  ...
  return <section className="filter-shell" aria-label="Параметры анализа">
    <div className="filter-bar">
      ...
      <div className="comparison-field"><span id="comparison-field-label">Объекты сравнения</span><button className="comparison-trigger" type="button" onClick={() => setMallSelector(true)} aria-haspopup="dialog" aria-labelledby="comparison-field-label comparison-field-value"><span id="comparison-field-value">{context.peerMalls.length} выбрано</span><Check size={16} /></button></div>
    </div>
