...
export function MallSheet({ mallName, data, context, onClose }: { mallName: string; data: DashboardData; context: AnalysisContext; onClose: () => void }) {
  ...
  return <div className="sheet-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}><div className="sheet" role="dialog" aria-modal="true" aria-labelledby="mall-sheet-title" tabIndex={-1} ref={panel}><header><div><p className="eyebrow">Карточка объекта</p><h2 id="mall-sheet-title">{stats.mall.mall}</h2><p className="sheet-subtitle">{stats.mall.city} · {stats.mall.mallClass}</p></div><Button variant="ghost" onClick={onClose} aria-label="Закрыть"><X /></Button></header>
    <dl className="detail-list"><div><dt>GLA</dt><dd>{stats.mall.glaConfirmed ? formatArea(stats.mall.gla) : 'н/д'}</dd></div><div><dt>GBA</dt><dd>{formatArea(stats.mall.gba)}</dd></div><div><dt>Бренды текущего среза</dt><dd>{stats.brandCount}</dd></div><div><dt>Категории</dt><dd>{stats.categoryCount}</dd></div><div><dt>Плотность</dt><dd>{stats.density10kGla == null ? 'н/д' : `${formatNumber.format(stats.density10kGla)} бренда на 10 000 м² GLA`}</dd></div><div><dt>Глобальная уникальность</dt><dd>{globalUnique} · {stats.brandCount ? formatPercent(globalUnique / stats.brandCount) : '0%'}</dd></div>{similarity ? <><div><dt>Жаккар с фокусным объектом</dt><dd>{formatPercent(similarity.jaccard)}</dd></div><div><dt>Общие бренды</dt><dd>{similarity.common}</dd></div></> : null}<div><dt>Покрытие ссылками</dt><dd>{rows.length ? formatPercent(sourceRows.length / rows.length) : '0%'}</dd></div></dl>
    ...
    <h3>Значения ниже медианы</h3>{belowMedian.length ? <div className="mall-category-list">{belowMedian.map((item) => <div key={item.category}><span>{item.category}</span><small>{item.value} против медианы {formatNumber.format(item.median)}</small></div>)}</div> : <p className="empty-state compact">В текущем срезе нет категорий ниже медианы группы сравнения.</p>}
